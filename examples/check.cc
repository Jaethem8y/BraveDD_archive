...
int main(int argc, const char **argv) {
  ...
  Func resU = compute_saturation(forest1, res1, relations);
  Func resR = compute_saturation(forestC, resC, relations);
  Func comp(forestC);
  apply(COPY, resU, comp);
  if (resU.getEdge().getEdgeHandle() != comp.getEdge().getEdgeHandle()) {
    std::cout << "THIS IS A FAILURE" << std::endl;
  } 
